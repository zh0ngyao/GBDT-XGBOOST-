...
class ChurnPredWithGBDTAndLR:
    ...
    # 拆分数据集
    def split(self):
        train, test = train_test_split(self.data, test_size=0.3, random_state=10)
        return train, test
    ...
    def evaluate(self,enc,gbdt,gbdt_lr):
        ...
        for y in gbdt_y_pred:
            ...
            new_gbdt_y_pred.append(1 if y[1] > 0.9 else 0)

        print("GBDT-MAE: %.4f" % mean_absolute_error(y_test, new_gbdt_y_pred))
        ...
        for y in gbdt_lr_y_pred:
            # y[0] 表示样本label=0的概率 y[1]表示样本label=1的概率
            new_gbdt_lr_y_pred.append(1 if y[1] > 0.9 else 0)

        print("GBDT_LR-MAE: %.4f" % mean_absolute_error(y_test, new_gbdt_lr_y_pred))
